...
public class TestimonialHateosRouterFunction {
    ...
    protected Mono testimonialRoutesHateos(@PathVariable("page") final int page, @PathVariable("size") final int size) {
        ...
        return service
                ...
                .map(testimonial ->
                    Pair.of(testimonial,
                        Flux.concat(
                            linkTo(controller.delete(testimonial.getId()))
                            .withSelfRel()
                            .toMono())
                    ))
                ...
                .collectList()
                .flatMap(listModel -> linkTo(controller.testimonialRoutesHateos(page, size)).withSelfRel().toMono()
